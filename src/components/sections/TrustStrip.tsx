...
function Fact({ value, label }: { value: string; label: string }) {
  return (
    <span className="flex items-baseline gap-3 whitespace-nowrap px-6">
      <span className="font-[family-name:var(--font-display)] text-[15px] font-bold uppercase tracking-[-0.01em] text-chalk">
        {value}
      </span>
...
export function TrustStrip() {
  ...
  return (
    <section aria-label="Коротко про Crazy Fitness" className="border-y border-steel bg-graphite">
      ...
      <div className="shell hidden lg:block">
        <ul className="flex items-stretch justify-between divide-x divide-steel py-7">
          {trustFacts.map((fact) => (
            <li key={fact.value} className="flex-1 px-8 first:pl-0 last:pr-0">
              <p className="font-[family-name:var(--font-display)] text-[18px] font-bold uppercase tracking-[-0.01em] text-chalk">
                {fact.value}
              </p>
